...
public class ExcelUtils {
    //Этот метод читает Excel‑файл и печатает всё его содержимое в консоль
    public static void printText(String filePath) {
        try (InputStream fis = new FileInputStream(filePath);
             //объект Excel‑файла (XLSX), который Apache POI умеет читать
            ...
            for (Row row : sheet) {
                for (Cell cell : row) {
                    ...
                    switch (cell.getCellType()) {
                        ...
                        case NUMERIC -> {
                            //DateUtil.isCellDateFormatted() проверяет, является ли ячейка датой
